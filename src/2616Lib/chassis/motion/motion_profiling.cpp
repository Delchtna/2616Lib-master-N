    ...
    poslog<<"Timestamp,odom/actual_x,odom/actual_y,odom/actual_theta,odom/target_x,odom/target_y,odom/target_theta,odom/actual_ang_vel,odom/target_ang_vel"<<"\n";
      poslog.close();
      poslog.open("/usd/odom.csv", std::ios::out | std::ios::app);
      }
      void Motion_Profiling::log_iteration(double angvel) {
      poslog << (pros::millis()/1000.00) << ","
              << chassis.get_pose().x << "," << chassis.get_pose().y << ","
              << chassis.get_pose().angle << ","
              << current_point.pose.x << "," << current_point.pose.y << ","
              << current_point.pose.angle << "," << angvel<<","<<current_point.angular_velocity<<"\n";
        poslog.flush();
  }
    ...
    void Motion_Profiling::calculate_wheel_speeds() {
      ...
      log_iteration(current_angular_velocity);
